    ...
    with Device(host=device_ip, user=username, password=password, gather_facts=False, normalize=True) as dev:
        try:
            ...
            for item in interface_diagnostice_optics_information:
                ...
                """
                This checks for multiple RX sections as some card/optic combos show a different 
                xml selection
                """
                ...
                elif item.find('.//optics-diagnostics/laser-rx-optical-power-dbm') is not None:
                    rx_signal_avg_optical_power_dbm = item.find('.//optics-diagnostics/laser-rx-optical-power-dbm').text
                else:
